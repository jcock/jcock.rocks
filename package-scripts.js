...
const cssTasks = (src, dest) => {
  const getTask = (isProd) => {
    return `postcss -d ${dest} ${esc(join(src, '*.css'))} --config ${esc(join(src, 'postcss.config.js'))} ${isProd ? '--no-map' : ''}`;
  };
...
const hugoTasks = (src, dest) => {
  ...
  const cfg = join(src, 'config.toml');
...
module.exports = {
  scripts: {
    ...
    css: cssTasks(join(relSrc, 'css'), join(relDest, 'css')),
    hugo: hugoTasks(rel('hugo'), relDest),
    js: jsTasks(join(relSrc, 'js')),
    svg: svgTasks(join(relSrc, 'img'), join(relDest, 'img')),
    build: {
      clean: pub(
        `rimraf ${relDest}`,
